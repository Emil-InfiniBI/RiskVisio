...
if (process.env.NODE_ENV === 'production') {
  ...
  try {
    ...
    console.error('❌ Failed creating persistent data directory:', e?.message || e);
  }
}
...
if (fs.existsSync(distIndex)) {
  ...
  // No SPA bundled in this deployment (API-only). Provide a friendly root response.
  app.get('/', (_req, res) => {
    res.json({
      app: 'RiskVisio API',
      status: 'running',
      health: '/health',
      note: 'No frontend assets found in this deployment.'
    });
  });
}
